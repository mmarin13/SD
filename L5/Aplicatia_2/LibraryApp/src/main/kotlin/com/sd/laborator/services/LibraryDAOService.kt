...
class LibraryDAOService: LibraryDAO {
    ...
    override fun findAllByAuthor(author: String): Set<Book> {
        ...
        return (this.books.filter { book -> words.any { word -> book.hasAuthor(word) } }).toSet()
    }

    override fun findAllByTitle(title: String): Set<Book> {
        val words = title.split(" ")
        return (this.books.filter { book -> words.any { word -> book.hasTitle(word) } }).toSet()
    }

    override fun findAllByPublisher(publisher: String): Set<Book> {
        val words = publisher.split(" ")
        return (this.books.filter { book -> words.any { word -> book.publishedBy(word) } }).toSet()
    }
}